...
def draw_dash(param_dict: list, test_func: Callable) -> object:
    ...
    def add_smth_new(a_plot, r_plot, value, children, filename):
        ...
        if n_plot > int(value):
            ...
            fig_dropdown_second = add_dropdown(n_plot, type_drop='filter_second_ax', style_dropdown=dict(display='none'))

            s_a = dcc.Checklist(id={'type': 'second_ax', 'index': n_plot}, options=[{'label': 'Second ax', 'value': 's_a'}])
    ...
    def name_to_drop_down(filename, exist_opt):
        ...
        return [exist_opt[0] for i in range(len(exist_opt))], [exist_opt[0] for i in range(len(exist_opt))]
